	...
	class Debug {
		...
		function output_to_log( $log = '', string $email = '' ): void {
			...
			// translators: 1: plugin display name, 2: name of debug function, 3: PHP End of Line character, 4: name of function triggering debug function, 5: debug log message
				esc_html__( '%1$s - Message from %2$s():%3$s%4$s%5$s', 'sharizard-wordpress' ),
			...
			if ( ! empty( $email ) ) {
				...
				if ( $mail_sent ) {
					$message = esc_html_x( 'Email sent.', 'Successfully emailed from output_to_log()', 'sharizard-wordpress' ) . ' ' . $message;
				} else {
					$message = esc_html_x( 'Email attempted but failed.', 'Unsuccessfully emailed from output_to_log()', 'sharizard-wordpress' ) . ' ' . $message;
				}
			}
