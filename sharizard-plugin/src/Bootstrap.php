...
use SharizardWordpress\Core as Core;
	...
	class Bootstrap {
		...
		public function notice_old_php_version(): void {
			...
			$message = sprintf(
				...
				__( '%1$s requires at least PHP version %2$s in order to work. You have version %3$s. Please see %4$s for more information.', 'sharizard-wordpress' ),
		...
		public function notice_missing_required_theme(): void {
			...
			if ( ! empty( $this->required_theme['child'] ) ) {
				$child_message = sprintf(
					...
					__( ' and %1$s child theme ', 'sharizard-wordpress' ),
			...
			$message = sprintf(
				...
				__( 'The %1$s plugin requires the %2$s parent theme%3$sin order to work.%4$s', 'sharizard-wordpress' ),
				'<strong>' . PluginData::get_plugin_display_name() . '</strong>',
